...
# Obtendo as medias de dispersão da renda e do valor emprestado
variancia_renda = np.var(array_financeira_renda)
distancia_var_renda = variancia_renda / (media_renda**2) #Fórmula matemática para calcular a distancia
desvio_padrao_renda = np.std(array_financeira_renda)
coeficiente_var_renda = desvio_padrao_renda / media_renda

# Obtendo as medias de dispersão do valor emprestado
variancia_vlr_emprestado = np.var(array_financeira_Vlr_emprestado)
distancia__vlr_emprestados = variancia_vlr_emprestado / (media_Vlr_emprestado**2) #Fórmula matemática para calcular a distancia
...
print(f"A variância empréstimos dos clientes é {variancia_vlr_emprestado:.2f}")
print(f"A distância da variância X média dos empréstimos dos clientes é {distancia__vlr_emprestados:.2f}")
print(f"O desvio padrão dos empréstimos dos clientes é {desvio_padrao_vlr_emprestado:.2f}")
print(f"O coeficiente de variação dos empréstimos dos clientes é {coeficiente_vlr_emprestado:.2f}")
...
plt.suptitle("Analise dos Dados sobre Renda X Empréstimo")
...
plt.subplot(2,2,2)
plt.title("Lista das Empréstimos")
...
plt.text(0.1,0.4,f'Distância entre a Variância e Média dos Empréstimos {distancia_Vlr_emprestado}',fontsize=12)
plt.text(0.1,0.3,f'Coeficiente de variação dos Empréstimos {coeficiente_var_renda}',fontsize=12)
